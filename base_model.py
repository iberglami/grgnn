...
class GetRNNOutput(nn.Module):#RNN new
    def forward(self, x):
        out, _ = x
        return out
...
class Model(nn.Module):
    def __init__(self, units, stack_cnt, time_step, multi_layer, horizon=1, dropout_rate=0.3, leaky_rate=0.2,
                 device='cpu',hidden_size = 40, num_layers = 1):
        super(Model, self).__init__()
        self.unit = units
        self.hidden_size = hidden_size##为GRU新加
        self.num_layers = num_layers##为GRU新加
        self.stack_cnt = stack_cnt
        self.unit = units
        ...
        self.fc = nn.Sequential(
            ...
            nn.LeakyReLU(),
            nn.GRU(int(self.time_step), int(self.time_step), int(self.num_layers), batch_first = True),
            GetRNNOutput(),
            nn.Linear(int(self.hidden_size), self.horizon),
        )

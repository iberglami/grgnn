...
def validate(model, dataloader, device, normalize_method, statistic,
    ...
    print(f'NORM: MAE {score_norm[1]:7.3f}.')
    print(f'RAW : MAE {score[1]:7.3f}。')
    if result_file:
    ...
    return dict(mae=score[1], mae_node=score_by_node[1])
...
def test(test_data, args, result_train_file, result_test_file):
    ...
    print('Performance on test set:  MAE: {:5.2f}'.format(mae))